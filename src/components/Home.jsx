    ...
    <div className="bg-white dark:bg-slate-800 border border-gray-200 dark:border-gray-700 rounded-2xl p-6 shadow-md hover:shadow-xl transition-all duration-300 flex flex-col group relative overflow-hidden">
      ...
      <div className="flex items-start justify-between mb-4 pl-2">
        <div>
          ...
          <h3 className="text-lg font-bold text-gray-900 dark:text-white leading-tight pr-4 group-hover:text-purple-600 transition-colors">
            {idea.title}
          </h3>
        </div>
        <button
          onClick={copyIdea}
          className="copy-btn shrink-0 text-gray-500 hover:text-purple-600 bg-gray-50 hover:bg-gray-100 dark:bg-gray-800 dark:hover:bg-gray-700 p-2 rounded-lg transition"
          title="Copy Idea"
        >
          {copied ? "✅" : "📋"}
        </button>
      ...
      <div className="flex flex-wrap gap-2 mb-5 pl-2">
        <span className="text-[11px] font-medium flex items-center gap-1.5 bg-purple-50 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300 px-2.5 py-1 rounded-md border border-purple-100 dark:border-purple-800/50">
          📱 {idea.platform}
        </span>
        <span className="text-[11px] font-medium flex items-center gap-1.5 bg-pink-50 dark:bg-pink-900/30 text-pink-700 dark:text-pink-300 px-2.5 py-1 rounded-md border border-pink-100 dark:border-pink-800/50">
          🎬 {idea.format}
        </span>
      </div>
      ...
      <div className="space-y-4 flex-grow text-sm pl-2">
        <div className="bg-gray-50 dark:bg-slate-900 p-3.5 rounded-xl border-l-2 border-purple-600 shadow-inner">
          <p className="text-[10px] text-gray-500 dark:text-gray-400 font-bold mb-1.5 uppercase tracking-wider">
            🎯 Opening Hook
          </p>
          <p className="font-medium italic text-gray-800 dark:text-gray-200">
            "{idea.hook}"
          </p>
        </div>
        ...
        <div>
          <p className="text-[10px] text-gray-500 dark:text-gray-400 font-bold mb-2.5 uppercase tracking-wider">
            # Trending Hashtags
          </p>
          <div className="flex flex-wrap gap-2">
            {safeHashtags.map((h, i) => (
              <span
                key={i}
                className="bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 text-xs px-3 py-1 rounded-full font-medium border border-gray-200 dark:border-gray-700 hover:text-purple-600 dark:hover:text-purple-400 transition-colors cursor-default"
              >
                {h}
              </span>
            ))}
        ...
        <div className="bg-gradient-to-br from-purple-600/5 to-pink-500/5 dark:from-purple-600/10 dark:to-pink-500/10 border border-purple-600/20 p-4 rounded-xl text-center mt-4">
          ...
          <p className="font-black text-transparent bg-clip-text bg-gradient-to-r from-purple-600 to-pink-500 text-lg leading-tight uppercase">
      ...
      <div className="mt-5 pt-5 border-t border-gray-100 dark:border-gray-800 pl-2">
        <button
          ...
          className="w-full py-2.5 bg-gray-50 hover:bg-gray-100 dark:bg-gray-800 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-300 border border-gray-200 dark:border-gray-700 font-semibold rounded-xl transition text-sm flex items-center justify-center gap-2"
        >
        ...
        {scriptOpen && (
          <div className="mt-3 bg-gray-50 dark:bg-slate-900 border border-gray-200 dark:border-gray-700 rounded-xl p-4 text-sm space-y-4 shadow-inner">
            <h4 className="font-bold text-cyan-400 text-xs uppercase tracking-widest mb-3 text-center border-b border-gray-200 dark:border-gray-700 pb-2">
            ...
            <div>
              <span className="block text-[10px] font-bold text-pink-500 mb-1 uppercase tracking-wider">
                Content
              </span>
              <p className="text-gray-700 dark:text-gray-300 leading-relaxed">
                {safeScript.content}
            ...
            <div>
              <span className="block text-[10px] font-bold text-cyan-400 mb-1 uppercase tracking-wider">
                Call to Action
              </span>
              <p className="text-gray-700 dark:text-gray-300 leading-relaxed">
                {safeScript.cta}
...
export default function Home() {
  // Read initial theme: prefer localStorage, then system preference
  ...
  const [keyStatus, setKeyStatus] = useState(
    localStorage.getItem("groq_api_key")
      ? {
          msg: "API key loaded. Ready to generate!",
  ...
  // Apply theme on first mount synchronously so no flash
  useEffect(() => {
  ...
  function toggleTheme() {
    setTheme((prev) => {
      ...
      // Apply immediately to avoid any lag
  ...
  function saveKey() {
    ...
    if (!val.startsWith("gsk_")) {
      setKeyStatus({
        msg: "Key should start with gsk_",
        cls: "text-red-500 text-sm",
      });
      return;
    }
  ...
  async function fetchTrends() {
    if (!apiKey) {
      setError("Please set your Groq API key first in settings.");
      setKeyModalOpen(true);
      return;
    }
    if (!topic) {
      setError("Enter a Topic/Niche first to detect trends.");
      return;
    }
    setError("");
            ...
            messages: [
              {
                role: "system",
                content:
                  "You are a viral trend analyst. Always return valid JSON only.",
              },
  ...
  async function generateIdeas() {
    ...
    if (!topic) {
      setError("Please enter a Topic/Niche.");
      return;
    }
    setError("");
            ...
            messages: [
              {
                role: "system",
                content:
                  "You are a viral social media content expert. Always respond with valid JSON only, no explanations.",
              },
              { role: "user", content: prompt },
            ],
            temperature: 0.9,
    ...
    <div className="bg-gray-50 dark:bg-slate-900 text-gray-900 dark:text-white font-sans min-h-screen flex flex-col transition-colors duration-300">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center gap-2">
              ...
              <span className="font-bold text-lg tracking-tight bg-clip-text text-transparent bg-gradient-to-r from-purple-600 to-pink-500">
        ...
        <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-800 border border-gray-200 dark:border-gray-700 p-6 rounded-2xl w-full max-w-md shadow-2xl relative">
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">
              You need a free Groq API key to generate ideas.
            </p>
            <input
              type="password"
              value={apiKeyInput}
            ...
            <button
              onClick={saveKey}
              className="w-full bg-gradient-to-r from-purple-600 to-pink-500 hover:opacity-90 text-white font-semibold py-2.5 rounded-lg transition shadow-md hover:shadow-lg"
            >
              Save Key
            </button>
            {keyStatus && (
              <div
                className={`mt-3 text-sm text-center font-medium ${keyStatus.cls}`}
              >
                {keyStatus.msg}
              </div>
            )}
      ...
      <main className="flex-grow max-w-5xl mx-auto w-full px-4 py-8">
        ...
        <section className="mb-10">
          ...
          <div
            className="flex overflow-x-auto gap-3 pb-2 py-1"
            ...
            {trends.length === 0 ? (
              ...
              trends.map((t, i) => (
                <button
                  key={i}
                  onClick={() => setTopic(t)}
                  className="whitespace-nowrap bg-white dark:bg-slate-800 border border-gray-200 dark:border-gray-700 hover:border-purple-600 dark:hover:border-purple-600 hover:text-purple-600 dark:hover:text-purple-400 px-4 py-2 rounded-full text-sm font-medium transition cursor-pointer shadow-sm text-gray-700 dark:text-gray-300"
                >
                  {t}
                </button>
              ))
        ...
        {/* HERO INPUT */}
        <section className="bg-white dark:bg-slate-800 border border-gray-200 dark:border-gray-800 rounded-3xl p-6 md:p-10 shadow-xl dark:shadow-2xl mb-12 relative overflow-hidden transition-colors duration-300">
          ...
          <div className="relative z-10 text-center mb-10">
            ...
            <p className="text-gray-500 dark:text-gray-400 text-lg">
              AI-powered concepts tailored for maximum virality.
            </p>
          </div>
              ...
              <div key={id}>
                ...
                <div className="relative">
                  <select
                    ...
                    className="w-full bg-gray-50 dark:bg-slate-900 border border-gray-300 dark:border-gray-700 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-purple-600 focus:ring-2 focus:ring-purple-600/50 transition appearance-none cursor-pointer"
                  >
                    {options.map((o) =>
                      typeof o === "string" ? (
          ...
          {error && (
            <div className="mt-6 p-4 bg-red-50 dark:bg-red-500/10 border border-red-200 dark:border-red-500/20 text-red-600 dark:text-red-400 text-sm rounded-xl text-center font-medium">
              {error}
            </div>
          )}
        </section>
